...
func (s *Store) GetRules() motif.Rules {
	cfg := s.rlp.Get(s.table.Rules, []byte("c"), &motif.Rules{}).(*motif.Rules)
	return *cfg
}

func (s *Store) SetRules(cfg motif.Rules) {
	s.rlp.Set(s.table.Rules, []byte("c"), &cfg)
}

func (s *Store) GetGenesis() motif.Genesis {
	meatadata := s.GetMetadata()
	return motif.Genesis{
