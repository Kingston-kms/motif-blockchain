...
func BenchmarkFlushDBs(b *testing.B) {
	...
	genStore := makegenesis.FakeGenesisStore(1, utils.ToMotif(1), utils.ToMotif(1))
	_, _, store, s2, s3, _ := MakeEngine(rawProducer, InputGenesis{
		...
		Motif:         gossip.DefaultConfig(cachescale.Identity),
		MotifStore:    gossip.DefaultStoreConfig(cachescale.Identity),
