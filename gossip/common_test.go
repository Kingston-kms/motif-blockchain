...
func newTestEnv() *testEnv {
	genStore := makegenesis.FakeGenesisStore(genesisStakers, utils.ToMotif(genesisBalance), utils.ToMotif(genesisStake))
...
func (env *testEnv) callContract(
	...
	vmenv := vm.NewEVM(context, txContext, state, env.store.GetRules().EvmChainConfig(), motif.DefaultVMConfig)
