...
//go:generate bash -c "cd ../../motif-sfc && git checkout c1d33c81f74abf82c0e22807f16e609578e10ad8"
//go:generate bash -c "docker run --name go-motif-sfc-compiler -v $(pwd)/contract/solc:/src/build/contracts -v $(pwd)/../../motif-sfc:/src -w /src node:10.5.0 bash -c 'export NPM_CONFIG_PREFIX=~; npm install --no-save; npm install --no-save truffle@5.1.4' && docker commit go-motif-sfc-compiler go-motif-sfc-compiler-image && docker rm go-motif-sfc-compiler"
//go:generate bash -c "docker run --rm -v $(pwd)/contract/solc:/src/build/contracts -v $(pwd)/../../motif-sfc:/src -w /src go-motif-sfc-compiler-image bash -c 'export NPM_CONFIG_PREFIX=~; rm -f /src/build/contracts/*json; npm run build'"
...
//go:generate bash -c "docker run --rm -v $(pwd)/contract/solc:/src/build/contracts -v $(pwd)/../../motif-sfc:/src -w /src go-motif-sfc-compiler-image bash -c 'export NPM_CONFIG_PREFIX=~; sed -i s/runs:\\ 200,/runs:\\ 10000,/ /src/truffle-config.js; rm -f /src/build/contracts/*json; npm run build'"
//go:generate bash -c "cd ../../motif-sfc && git checkout -- truffle-config.js; docker rmi go-motif-sfc-compiler-image"
...
import (
	...
	"github.com/motifd/motif-blockchain/gossip/contract/driver100"
	"github.com/motifd/motif-blockchain/gossip/contract/driverauth100"
	"github.com/motifd/motif-blockchain/gossip/contract/netinit100"
	"github.com/motifd/motif-blockchain/gossip/contract/sfc100"
	"github.com/motifd/motif-blockchain/logger"
...
func TestSFC(t *testing.T) {
	...
	_ = true &&
		...
		t.Run("SFC upgrade", func(t *testing.T) {
			...
			rr := env.ApplyBlock(nextEpoch,
				env.Contract(admin, utils.ToMotif(0), sfc100.ContractBin),
			)
	...
	t.Run("Direct driver", func(t *testing.T) {
		...
		rr := env.ApplyBlock(nextEpoch,
			env.Contract(admin, utils.ToMotif(0), anyContractBin),
		)
		require.Equal(1, rr.Len())
		require.Equal(types.ReceiptStatusSuccessful, rr[0].Status)
		newImpl := rr[0].ContractAddress
...
func cicleTransfers(t *testing.T, env *testEnv, count uint64) {
	...
	for i := uint64(0); i < count; i++ {
		...
		for i := range txs {
			from := (i)%accounts + 1
			to := (i+1)%accounts + 1
			txs[i] = env.Transfer(from, to, utils.ToMotif(100))
		}
