...
type MotifEpochsSealerModule struct{}

func New() *MotifEpochsSealerModule {
	return &MotifEpochsSealerModule{}
}

func (m *MotifEpochsSealerModule) Start(block blockproc.BlockCtx, bs blockproc.BlockState, es blockproc.EpochState) blockproc.SealerProcessor {
	return &MotifEpochsSealer{
		block: block,
		es:    es,
		bs:    bs,
	}
...
type MotifEpochsSealer struct {
	block blockproc.BlockCtx
	es    blockproc.EpochState
	bs    blockproc.BlockState
}

func (s *MotifEpochsSealer) EpochSealing() bool {
...
func (p *MotifEpochsSealer) Update(bs blockproc.BlockState, es blockproc.EpochState) {
	p.bs, p.es = bs, es
}

// SealEpoch is called after pre-internal transactions are executed
func (s *MotifEpochsSealer) SealEpoch() (blockproc.BlockState, blockproc.EpochState) {
