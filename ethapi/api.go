...
// It offers only methods that motifte on public data that is freely available to anyone.
...
// PublicTxPoolAPI offers and API for the transaction pool. It only motiftes on data that is non confidential.
...
func DoEstimateGas(ctx context.Context, b Backend, args CallArgs, blockNrOrHash rpc.BlockNumberOrHash, gasCap uint64) (hexutil.Uint64, error) {
	...
	executable := func(gas uint64) (bool, *evmcore.ExecutionResult, error) {
		...
		result, err := DoCall(ctx, b, args, blockNrOrHash, nil, motif.DefaultVMConfig, 0, gasCap)
		if err != nil {
...
func SubmitTransaction(ctx context.Context, b Backend, tx *types.Transaction) (common.Hash, error) {
	...
	if tx.To() == nil {
		...
		log.Info("Submitted transaction", "hash", tx.Hash().Hex(), "from", from, "nonce", tx.Nonce(), "recipient", tx.To(), "value", tx.Value(), "input", tx.Input(), )
	}
	return tx.Hash(), nil
}
...
func checkTxFee(gasPrice *big.Int, gas uint64, cap float64) error {
	...
	if feeFloat > cap {
		return fmt.Errorf("tx fee (%.2f MOTIF) exceeds the configured cap (%.2f MOTIF)", feeFloat, cap)
	}
	return nil
}
