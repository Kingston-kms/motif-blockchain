...
func TestConsoleWelcome(t *testing.T) {
	// Start a motif console, make sure it's cleaned up and terminate the console
	cli := exec(t,
...
instance: go-motif/v{{version}}/{{goos}}-{{goarch}}/{{gover}}
coinbase: {{.Coinbase}}
at block: 1 ({{niltime}})
 datadir: {{.Datadir}}
 modules: {{apis}}
...
func TestIPCAttachWelcome(t *testing.T) {
	...
	if runtime.GOOS == "windows" {
		...
		ipc = filepath.Join(ws, "motif.ipc")
	}
	cli := exec(t,
		"--fakenet", "0/1", "--port", "0", "--maxpeers", "0", "--nodiscover", "--nat", "none",
		"--ipcpath", ipc)
...
func testAttachWelcome(t *testing.T, cli *testcli, endpoint, apis string) {
	// Attach to a running motif node and terminate immediately
