...
import (
	...
	"github.com/motifd/motif-blockchain/gossip"
	"github.com/motifd/motif-blockchain/gossip/gasprice"
	"github.com/motifd/motif-blockchain/integration"
	"github.com/motifd/motif-blockchain/integration/makegenesis"
...
var (
	...
	RPCGlobalTxFeeCapFlag = cli.Float64Flag{
		Name:  "rpc.txfeecap",
		Usage: "Sets a cap on transaction fee (in MOTIF) that can be sent via the RPC APIs (0 = no cap)",
		Value: gossip.DefaultConfig(cachescale.Identity).RPCTxFeeCap,
	}

	AllowedMotifGenesisHashes = map[uint64]hash.Hash{
		motif.MainNetworkID: hash.HexToHash("0x2d8963b2c8a028086acc794f7291025a833a3e17d8a097334cfe86769bfd7ecb"),
		motif.TestNetworkID: hash.HexToHash("0x2d8963b2c8a028086acc794f7291025a833a3e17d8a097334cfe86769bfd7ecb"),
	}
)
...
func (c *config) AppConfigs() integration.Configs {
	return integration.Configs{
		Motif:          c.Motif,
		MotifStore:     c.MotifStore,
		Lachesis:       c.Lachesis,
		LachesisStore:  c.LachesisStore,
		VectorClock:    c.VectorClock,
		AllowedGenesis: AllowedMotifGenesisHashes,
	}
}
...
func getMotifGenesis(ctx *cli.Context) integration.InputGenesis {
...
func mayMakeAllConfigs(ctx *cli.Context) (*config, error) {
	...
	cfg := config{
		...
		Motif:         gossip.DefaultConfig(cacheRatio),
		MotifStore:    gossip.DefaultStoreConfig(cacheRatio),
	...
	if ctx.GlobalIsSet(FakeNetFlag.Name) {
		...
		cfg.Motif = gossip.FakeConfig(num, cacheRatio)
	}
	...
	var err error
	cfg.Motif, err = gossipConfigWithFlags(ctx, cfg.Motif)
	if err != nil {
		return nil, err
	}
	cfg.MotifStore, err = gossipStoreConfigWithFlags(ctx, cfg.MotifStore)
	if err != nil {
		return nil, err
	}
	cfg.Node = nodeConfigWithFlags(ctx, cfg.Node)
	if cfg.Motif.Emitter.Validator.ID != 0 && len(cfg.Motif.Emitter.PrevEmittedEventFile.Path) == 0 {
		cfg.Motif.Emitter.PrevEmittedEventFile.Path = cfg.Node.ResolvePath(path.Join("emitter", fmt.Sprintf("last-%d", cfg.Motif.Emitter.Validator.ID)))
	}

	if err := cfg.Motif.Validate(); err != nil {
		return nil, err
	}
