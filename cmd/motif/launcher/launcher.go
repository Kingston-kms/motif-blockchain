...
import (
	...
	"github.com/motifd/motif-blockchain/cmd/motif/launcher/metrics"
	"github.com/motifd/motif-blockchain/cmd/motif/launcher/tracing"
	"github.com/motifd/motif-blockchain/debug"
	"github.com/motifd/motif-blockchain/flags"
	"github.com/motifd/motif-blockchain/gossip"
	"github.com/motifd/motif-blockchain/integration"
	"github.com/motifd/motif-blockchain/utils/errlock"
	"github.com/motifd/motif-blockchain/valkeystore"
	_ "github.com/motifd/motif-blockchain/version"
)

const (
	// clientIdentifier to advertise over the network.
	clientIdentifier = "go-motif"
)
...
var (
	...
	app = flags.NewApp(gitCommit, gitDate, "the go-motif command line interface")
...
func initFlags() {
	...
	nodeFlags = append(nodeFlags, motifFlags...)
...
// motif is the main entry point into the system if no special subcommand is ran.
...
func lachesisMain(ctx *cli.Context) error {
	...
	genesisPath := getMotifGenesis(ctx)
...
func makeNode(ctx *cli.Context, cfg *config, genesis integration.InputGenesis) (*node.Node, *gossip.Service, func()) {
	...
	valPubkey := cfg.Motif.Emitter.Validator.PubKey
	if key := getFakeValidatorKey(ctx); key != nil && cfg.Motif.Emitter.Validator.ID != 0 {
	...
	svc, err := gossip.NewService(stack, cfg.Motif, gdb, signer, blockProc, engine, dagIndex)
	if err != nil {
...
func startNode(ctx *cli.Context, stack *node.Node) {
	...
	// Create a client to interact with local motif node.
