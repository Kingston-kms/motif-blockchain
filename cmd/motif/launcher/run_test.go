...
import (
	...
	"github.com/motifd/motif-blockchain/cmd/cmdtest"
)

func tmpdir(t *testing.T) string {
	dir, err := ioutil.TempDir("", "motif-test")
	if err != nil {
		t.Fatal(err)
	}
	return dir
...
func init() {
	// Run the app if we've been exec'd as "motif-test" in exec().
	reexec.Register("motif-test", func() {
...
func exec(t *testing.T, args ...string) *testcli {
	...
	// Boot "motif". This actually runs the test binary but the TestMain
	// function will prevent any tests from running.
	tt.Run("motif-test", args...)
