...
func ApplyGenesis(statedb *state.StateDB, g motif.Genesis, maxMemoryUsage int) (*EvmBlock, error) {
	mem := 0
...
func genesisBlock(g motif.Genesis, root common.Hash) *EvmBlock {
	block := &EvmBlock{
		EvmHeader: EvmHeader{
			Number:   big.NewInt(0),
			Time:     g.Time,
...
func MustApplyGenesis(g motif.Genesis, statedb *state.StateDB, maxMemoryUsage int) *EvmBlock {
